...
export function cloneTemplate<T extends HTMLElement>(query: string | HTMLTemplateElement): T {
    if (typeof query === 'string') {
        const template = document.querySelector(query) as HTMLTemplateElement;
        if (!template) {
            throw new Error(`Template with selector "${query}" not found`);
        }
    ...
    if (query instanceof HTMLTemplateElement) {
        if (!query.content.firstElementChild) {
            throw new Error(`Template has no content`);
        }
        return query.content.firstElementChild.cloneNode(true) as T;
    }
    throw new Error('Unknown template selector');
}
